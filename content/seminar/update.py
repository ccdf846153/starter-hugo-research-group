...
# final html path
HTML_PATH = os.path.realpath("../seminar_list/content.txt")
...
def get_AI_material_info():
    ...
    for material_dir in material_dir_list:
        ...
        material_file_list = list(
            filter(
                lambda x: x == "sort.dic"
                or (x.endswith((".pdf", ".ppt", "pptx", "txt")) and x.find("_") != -1),
                filename_list,
            )
        )
        ...
        for material_file in material_file_list:
            ...
            if material_file == "sort.dic":
                with open(
                    os.path.join(material_path, material_file), "r", encoding="utf-8"
                ) as sort_file:
    ...
    for material_dir in material_dir_list:
        ...
        for material_file in material_file_list:
            ...
            if material_file == "sort.dic":
                ...
                if (f"{title}_会议纪要.docx" in note_list) or (
                    f"{title}_会议纪要.doc" in note_list
                ):
                    button_list.append(
                        {
                            "type": "Note",
                            "path": (
                                f"{PARSE_NORMAL_PATH}/{material_dir}/{title}_会议纪要.docx"
                                if f"{title}_会议纪要.docx" in note_list
                                else f"{PARSE_NORMAL_PATH}/{material_dir}/{title}_会议纪要.doc"
                            ),
                        }
                    )
...
def set_seminar_table_html(info_list: list) -> BeautifulSoup:
    ...
    ret_html = table_head = txt_to_html(
        f"{TEMPLATE_PATH}\\seminar_table-head.txt", encoding="utf-8"
    )
    table_row_temp = txt_to_html(
        f"{TEMPLATE_PATH}\\seminar_table-row.txt", encoding="utf-8"
    )
    table_button_temp = txt_to_html(
        f"{TEMPLATE_PATH}\\seminar_table-button.txt", encoding="utf-8"
    )
    ...
    def material_sort_arg(sorted_list: list, kwd_list: list):
        ...
        return [
            [(kwd in x["presenters"]) for x in sorted_list].index(True)
            for kwd in kwd_list
        ]
    ...
    for material_list, sort_list in info_list:
        ...
        if sort_list is not None:
            material_list = [
                material_list[idx]
                for idx in material_sort_arg(material_list, sort_list)
            ][::-1]
        for material in material_list:

            table_row = table_row_temp.__copy__()
            table_row.find("tr").attrs["class"] = "year-{}".format(
                material["date"].split("-")[0]
            )

            table_row.find("span", {"class": "seminar-date"}).string = material["date"]
            table_row.find("span", {"class": "seminar-presenter"}).string = "、".join(
                material["presenters"]
            )
            table_row.find("span", {"class": "seminar-title"}).string = material[
                "title"
            ]
            table_row.find("span", {"class": "seminar-title"}).parent.attrs[
                "style"
            ] = f"max-width: {title_width(len(material['buttons']))}%"

            for button in sorted(
                material["buttons"], key=lambda x: button_display_order[x["type"]]
            ):
                table_button = table_button_temp.__copy__()
                table_button.find("a").attrs["href"] = button["path"]
                table_button.find("a").string = button["type"]

                table_row.find("td", {"class": "seminar-topic"}).insert(
                    -2, table_button
                )
